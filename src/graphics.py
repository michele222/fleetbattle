...
class Graphics:
    ...
    def __init__(self):
        ...
        self.attack_grid = pygame.Rect((2 * parameters.MARGIN + parameters.N * parameters.SQUARE,
                                        parameters.MARGIN,
                                        parameters.N * parameters.SQUARE,
                                        parameters.N * parameters.SQUARE))
        ...
        #explosion images
        self.hit_texture = {
            True : pygame.image.load(path_to_images.joinpath('explosion.png')).convert_alpha(),       #True: explosion on ship (hit)
            False : pygame.image.load(path_to_images.joinpath('explosion_sea.png')).convert_alpha()   #False: explosion at sea (miss)
        }
    ...
    def get_attack_position(self):
        ...
        if self.attack_grid.collidepoint(mouse_pos):
            return ((mouse_pos[0] - self.attack_grid.x) // parameters.SQUARE
                    + ((mouse_pos[1] - self.attack_grid.y) // parameters.SQUARE) * parameters.N)
        else:
            return -1   
    ...
    def get_space_for_ship_size(self, size):
        ...
        if self.placement_pos // parameters.N != (self.placement_pos + size) // parameters.N:
            self.placement_pos = (self.placement_pos // parameters.N + 1) * parameters.N
        ret = (self.attack_grid.topleft[0]
               + parameters.SQUARE
               * (self.placement_pos % parameters.N),
               self.attack_grid.topleft[1]
               + (parameters.SQUARE + parameters.MARGIN)
               * (self.placement_pos // parameters.N))
        self.placement_pos += (size + 1)         
        return ret
