...
class Ship:
    ...
    def __init__(self, length = 1, pos = (0, 0), horizontal = True):
        ...
        self.body = pygame.Rect((pos[0], pos[1], Parameters.SQUARE + horizontal * Parameters.SQUARE * (length - 1), Parameters.SQUARE + (1 - horizontal) * Parameters.SQUARE * (length - 1)))
        self.positionInit = pos
    
    def reset(self):
        self.horizontal = self.horizontalInit
        self.body = pygame.Rect((self.positionInit[0], self.positionInit[1], Parameters.SQUARE + self.horizontal * Parameters.SQUARE * (self.length - 1), Parameters.SQUARE + (1 - self.horizontal) * Parameters.SQUARE * (self.length - 1)))
